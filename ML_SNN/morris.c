...
int main(int argc, char *argv[])
{
	...
	int iter;
	 ...
	 float start_clock;
        ...
        for (i=0;i<row;i++)
        {
                input[i] = malloc(sizeof(float)*col);
        }
         float * orig_input = malloc(sizeof(float)*24*24);
         w=(float **)malloc(sizeof(float *)*Ni);

         float * orig_w = malloc(sizeof(float)*24*24*Ni);

        for (i=0;i<Ni;i++)
        {
                w[i] = malloc(sizeof(float)*Ne);
        }
	...
	float *new_weights;
        new_image = malloc(sizeof(float)*Ne);
        new_weights = malloc(sizeof(float)*Ne*Ni);
